...
# Below is graph serves as visual documentation of the tree generated by the parser.
#
# (expression
#   (sum
#     (number)
...
class VisitorTest < Minitest::Test
  ...
  def test_on_default_doesnt_run_with_custom_hook
    tree = @parser.parse_string(<<~MATH)
      1 + x * 3
    MATH
  ...
  def test_around_default
    ...
    visitor = TreeStand::Visitor.new(tree.root_node)
    visitor.define_singleton_method(:_around_default, method)
    visitor.visit
  ...
  def test_around_default_doesnt_run_with_custom_hook
    tree = @parser.parse_string(<<~MATH)
      1 + x * 3
    MATH
